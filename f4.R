####  Copyright (C) 2005, 2006, 2007, 2014, Ramon Diaz-Uriarte <rdiaz02@gmail.com>
...
runningProcs <- paste(appProcs, "/runs-tmp/R.running.procs", sep = "")
...
caughtUserError.Web <- ADaCGH2:::caughtUserError2
caughtOurError.Web <- ADaCGH2:::caughtOurError2

NormalTermination <- function(){
    ADaCGH2:::cluster.clean.quit.Web()
...
## this should not be necessary with the new ADaCGH2 facilities?
## i think yes, as this is for other processes that do not use
## ff, etc.
new.custom2 <- function(segmresRDataName = "segmres.RData",
  ...
  null <- mclapply(seqc, put.part.rdata.together,
                   chr.start, chr.end, mc.cores = detectCores())
...
if(checkpoint.num < 1) {
  ...
  ## chromnum <- max(tableChromArray$ChromNum)
    numarrays <- "not_queried"
    chromnum <- "not_queried"
...
library(parallel)
print(paste("Number of cores is ", detectCores()))

## library(Rmpi)
## print(system("lamnodes"))
## print(paste("Universe size is ", mpi.universe.size()))

## usize <- min(numarrays * chromnum, detectCores())
